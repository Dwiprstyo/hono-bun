...
// Update User Profile
app.put('/profile', authMiddleware, zValidator('json', ProfileUpdateSchema), async (c) => {
    try {
...
// Get User's Posts
app.get('/profile/:id/posts', authMiddleware, async (c) => {
    try {
        const userId = Number(c.req.param('id'));
...
app.get('/profile/:id/followers', authMiddleware, async (c) => {
    try {
        const userId = Number(c.req.param('id'));
        const page = Number(c.req.query('page') || '1');
        const limit = Number(c.req.query('limit') || '10');
...
app.get('/profile/:id/following', authMiddleware, async (c) => {
    try {
        const userId = Number(c.req.param('id'));
        const page = Number(c.req.query('page') || '1');
        const limit = Number(c.req.query('limit') || '10');
