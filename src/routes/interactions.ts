...
// Likes Routes
app.post('/likes', authMiddleware, zValidator('json', LikeSchema), async (c) => {
    try {
...
app.delete('/likes/:userId/:postId', authMiddleware, async (c) => {
    try {
...
// Comments Routes
app.post('/comments', authMiddleware, zValidator('json', CommentSchema), async (c) => {
    try {
        const data = c.req.valid('json');
...
app.put('/comments/:id', authMiddleware, zValidator('json', UpdateCommentSchema), async (c) => {
    try {
...
// Follow Routes
app.post('/follow', authMiddleware, zValidator('json', FollowSchema), async (c) => {
    try {
        const data = c.req.valid('json');
...
app.delete('/unfollow/:followerId/:followingId', authMiddleware, async (c) => {
    try {
